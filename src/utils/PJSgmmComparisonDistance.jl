...
function plot_distance(tid, rid, outputpath::String="Figures/Distance/")
    ...
    for i in 1:length(r_epi)
        Sai_esp_LLC[i] = PJSspainCrustal(Ti[tid], ruptures[rid], sites[i], "lower", "lower", "central").IM
        Sai_esp_LCC[i] = PJSspainCrustal(Ti[tid], ruptures[rid], sites[i], "lower", "central", "central").IM
        Sai_esp_LUC[i] = PJSspainCrustal(Ti[tid], ruptures[rid], sites[i], "lower", "upper", "central").IM
        Sai_esp_CLC[i] = PJSspainCrustal(Ti[tid], ruptures[rid], sites[i], "central", "lower", "central").IM
        Sai_esp_CCC[i] = PJSspainCrustal(Ti[tid], ruptures[rid], sites[i], "central", "central", "central").IM
        Sai_esp_CUC[i] = PJSspainCrustal(Ti[tid], ruptures[rid], sites[i], "central", "upper", "central").IM
        Sai_esp_ULC[i] = PJSspainCrustal(Ti[tid], ruptures[rid], sites[i], "upper", "lower", "central").IM
        Sai_esp_UCC[i] = PJSspainCrustal(Ti[tid], ruptures[rid], sites[i], "upper", "central", "central").IM
        Sai_esp_UUC[i] = PJSspainCrustal(Ti[tid], ruptures[rid], sites[i], "upper", "upper", "central").IM
    end
    ...
    theme(:default)
    p1 = plot(r_epi, Sai_esp_med, ribbon=(dSail_esp, dSaiu_esp), lab="Spain Crustal", box=:true, xaxis=:log10, yaxis=:log10, color=[1], linewidth=2)
    plot!(r_epi, Sai_esp_all, lab="", color=[1])
    plot!(r_epi, Sai_nga_med, ribbon=(dSail_nga, dSaiu_nga), lab="NGA West2", color=[2], linewidth=2)
    plot!(r_epi, Sai_nga_all, lab="", color=[2])
    plot!(r_epi, Sai_eur_med, ribbon=(dSail_eur, dSaiu_eur), lab="European", color=[3], linewidth=2)
    plot!(r_epi, Sai_eur_all, lab="", color=[3])
    xlabel!("Distance (km)")
    ylabel!("Spectral acceleration (g)")
    title!("T = $(Ti[tid])s, M = $(mi[rid])")


    p2 = plot(r_epi, Sai_esp_sd, lab="Spain Crustal", box=:true, color=[1], linewidth=2, xaxis=:log10)
    ...
    plot!(size=(980,580))
    outfile = string(outputpath,"PJSdistance_T$(replace(string(Ti[tid]),"."=>"p"))_M$(replace(string(mi[rid]),"."=>"p"))_v0.pdf")
    savefig(outfile)
    return p
end
