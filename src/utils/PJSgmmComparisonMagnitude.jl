...
function plot_magnitude(tid, sid, outputpath::String="Figures/Magnitude/")
    ...
    for i in 1:length(mi)
        Sai_esp_LLC[i] = PJSspainCrustal(Ti[tid], ruptures[i], sites[sid], "lower", "lower", "central").IM
        Sai_esp_LCC[i] = PJSspainCrustal(Ti[tid], ruptures[i], sites[sid], "lower", "central", "central").IM
        Sai_esp_LUC[i] = PJSspainCrustal(Ti[tid], ruptures[i], sites[sid], "lower", "upper", "central").IM
        Sai_esp_CLC[i] = PJSspainCrustal(Ti[tid], ruptures[i], sites[sid], "central", "lower", "central").IM
        Sai_esp_CCC[i] = PJSspainCrustal(Ti[tid], ruptures[i], sites[sid], "central", "central", "central").IM
        Sai_esp_CUC[i] = PJSspainCrustal(Ti[tid], ruptures[i], sites[sid], "central", "upper", "central").IM
        Sai_esp_ULC[i] = PJSspainCrustal(Ti[tid], ruptures[i], sites[sid], "upper", "lower", "central").IM
        Sai_esp_UCC[i] = PJSspainCrustal(Ti[tid], ruptures[i], sites[sid], "upper", "central", "central").IM
        Sai_esp_UUC[i] = PJSspainCrustal(Ti[tid], ruptures[i], sites[sid], "upper", "upper", "central").IM
    end
    ...
    theme(:default)
    p1 = plot(mi, Sai_esp_med, ribbon=(dSail_esp, dSaiu_esp), lab="Spain Crustal", box=:true, yaxis=:log10, color=[1], linewidth=2)
    plot!(mi, Sai_esp_all, lab="", color=[1])
    plot!(mi, Sai_nga_med, ribbon=(dSail_nga, dSaiu_nga), lab="NGA West2", color=[2], linewidth=2)
    plot!(mi, Sai_nga_all, lab="", color=[2])
    plot!(mi, Sai_eur_med, ribbon=(dSail_eur, dSaiu_eur), lab="European", color=[3], linewidth=2)
    plot!(mi, Sai_eur_all, lab="", color=[3])
    xlabel!("Magnitude")
    ylabel!("Spectral acceleration (g)")
    title!("T = $(Ti[tid]), Repi = $(r_epi[sid])km")


    p2 = plot(mi, Sai_esp_sd, lab="Spain Crustal", box=:true, color=[1], linewidth=2)
