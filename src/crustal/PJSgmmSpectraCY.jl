...
#r_epi = collect(range(0.0, stop=500, step=5.0))
# r_epi = [ 5.0, 15.0, 25.0, 40.0, 65.0, 100.0, 150.0, 250.0, 400.0, 650.0 ]
r_epi = [ 50.0, 100.0, 150.0, 200.0, 300.0, 400.0, 500.0, 600.0, 700.0, 800.0, 1000.0 ]
...
function plot_spectra(rid, sid, outputpath::String="Figures/SpectraCY/")
    ...
    Sai_cy14 = get_spectrum(gmi_cy14)
    ...
    theme(:default)
    p1 = plot(Ti, Sai_nga_med, ribbon=(dSail_nga, dSaiu_nga), lab="NGA West2", color=[1], linewidth=2, xaxis=:log10, yaxis=:log10)
    plot!(Ti, Sai_nga_all, lab="", color=[1])
    plot!(Ti, Sai_cy14, lab="", linewidth=4)
    xlabel!("Period (s)")
    ylabel!("Spectral acceleration (g)")
    title!("M = $(mi[rid]), Repi = $(r_epi[sid])km")


    p2 = plot(Ti, Sai_nga_sd, lab="NGA West 2", color=[1], linewidth=2, xaxis=:log10)
    ylims!(0,1.0)
    xlabel!("Period (s)")
    ...
    plot!(size=(980,580))
    outfile = string(outputpath,"PJSspectra_M$(replace(string(mi[rid]),"."=>"p"))_Repi$(replace(string(r_epi[sid]),"."=>"p"))_v0.pdf")
    savefig(outfile)
    return p
end
